...
function selectFaIcon(icon,event) {
  ...
  document.querySelector(".font-awesome-selector div").appendChild(clone);

}

// document.addEventListener('click', function(event) {
//   if (document.querySelectorAll("#fa-selector.active").length > 0) {
//     document.querySelectorAll("#fa-selector").forEach(function(el){
...
function createFaElements(event) {
  ...
  // insert icons
  populateFaIcons(false,event,faElCount);
...
function populateFaIcons(search,event,selectorNumber) {

  if (search == true) {
    iconList = faIconSearchResults;
    //bindSearchTimers()
  } else {
    iconList = icons.solid;
  }

  iconList.forEach(function(icon){

    // todo - define prefix per icon
    prefix = "fas"
...
function runFaIconSearch(event) {
  ...
  icons.solid.forEach(function(icon){
  ...
  populateFaIcons(true,event,selectorNumber);
